    ...
    T: Hash + Ord,
{
    ...
    pub fn merge(automata: &[Self]) -> Self
    where
        T: Clone,
    {
        ...
        let mut state_mapping = vec![HashMap::new(); automata.len()];
        let mut make_state = || {
    ...
    pub fn sequence(automata: &[Self]) -> Self
    where
        T: Clone,
    {
        let initial_state = State(0);
        let mut final_states = HashSet::from([initial_state]);
        let mut last_intermediate_state = initial_state;
        let mut transitions = HashMap::<_, HashMap<_, BTreeSet<_>>>::new();
        ...
        for (i, automaton) in automata.iter().enumerate() {
            state_mapping[i]
                .insert(automaton.initial_state, last_intermediate_state);
...
pub(crate) mod test {
    ...
    fn merge() {
        ...
        assert!(automaton.test(&[true, true, true, true]));
        assert!(automaton.test(&[true, false, false, true]));
        // This now is true due to the merging
        // assert!(!automaton.test(&[false, false, true]));
        // assert!(!automaton.test(&[false, true]));
        // assert!(!automaton.test(&[false, false, true, true]));
    }
